        ...
        where TShader : UnityEngine.Object
    {
        ...
        private static string FetchGlobalRenderPipelineShaderTag()
        {
            ...
            using (ListPool<RenderPipelineAsset>.Get(out List<RenderPipelineAsset> rpAssets))
            {
                if (EditorUserBuildSettings.activeBuildTarget.TryGetRenderPipelineAssets<RenderPipelineAsset>(rpAssets))
                {
                    ...
                    // DRAGON ECLIPSE FIX.
                    // Trong project nay URP runtime la DLL do AssetRipper trich ra, va ban trich do
                    // DA MAT phan ghi de `renderPipelineShaderTag`. Thuoc tinh roi ve lop cha
                    // RenderPipelineAsset (UnityEngine.CoreModule) tra ve chuoi rong. Hau qua:
                    // doan so sanh the RenderPipeline ben duoi thay "UniversalPipeline" != "" nen
                    // CAT SACH 100% bien the cua moi shader URP - ke ca Hidden/Universal/CoreBlit,
                    // shader ma URP dung de chep anh tu texture trung gian ra man hinh. Ban build
                    // vi vay chay dung logic nhung khong ve duoc gi (man hinh dung yen o khung
                    // hinh cuoi cua splash screen), con Editor thi binh thuong vi khong chay strip.
                    if (string.IsNullOrEmpty(globalRenderPipelineTag) && asset != null)
                    {
                        var typeName = asset.GetType().FullName ?? string.Empty;
                        if (typeName.Contains("Universal"))
                            globalRenderPipelineTag = "UniversalPipeline";
                        else if (typeName.Contains("HDRenderPipeline"))
                            globalRenderPipelineTag = "HDRenderPipeline";
                    }
                }
            }
        ...
        protected bool TryStripShaderVariants([DisallowNull] TShader shader, TShaderVariant shaderVariant, IList<ShaderCompilerData> compilerDataList, [NotNullWhen(false)] out Exception error)
        {
            ...
            if (typeof(TShader) == typeof(Shader) && typeof(TShaderVariant) == typeof(ShaderSnippetData))
            {
                ...
                {
                    // DRAGON ECLIPSE FIX: neu khong xac dinh duoc the cua pipeline dang dung thi
                    // KHONG duoc cat sach - "khong biet" phai la giu lai, khong phai vut het.
                    if (!string.IsNullOrEmpty(globalRenderPipeline) &&
                        !renderPipelineTag.Equals(globalRenderPipeline, StringComparison.OrdinalIgnoreCase))
